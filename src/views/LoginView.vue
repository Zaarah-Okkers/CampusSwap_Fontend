  ...
  <div class="login-page" :class="`login-${role}`">
    <div class="login-left">
      ...
      <h1 class="hero-title">
        ...
        <span class="gold-text"> Endless Possibilities. </span> All in One
        Place.
      </h1>
    ...
    <div class="login-right">
      <div class="form-wrapper">
        ...
        >
          ...
          <div class="select-wrap">
            ...
            <select v-model="selectedInstitution" class="form-input">
              ...
              <template v-if="role === 'student' || role === 'resmanager'">
                <optgroup
        ...
        <div class="form-options">
          <a
            href="#"
            @click.prevent="showPasswordResetNotice"
            class="forgot-link"
          >
        ...
        <div class="mobile-footer">
          <p>
            <AppIcon name="lock" />
            Ozow Escrow Protected &bull; Secured by EduID
          </p>
        </div>
      </div>
    </div>
    ...
    >
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div
            v-if="regData.role === 'student' || regData.role === 'resmanager'"
            class="form-group"
          >
          ...
          <div v-if="regData.role === 'student'" class="form-group">
            <label> Upload Student ID (required)</label>
            ...
            <div class="file-upload-wrapper">
              ...
              <p class="file-hint">
                You may upload a photo of your student ID card for later
                verification.
              </p>
            </div>
          </div>
          ...
          >
            <span class="verified-badge">Verification Pending </span>

            <p class="verified-text">
              >An admin will review your details and approve your account.
            </p>
          </div>
...
export default {
  ...
  methods: {
    ...
    persistLogin(user, role) {
      ...
      const displayName =
        user.full_name || user.fullName || user.name || "CampusSwap User";
      const account = {
    ...
    async handleRegistration() {
      ...
      if (!this.regData.fullName)
        return this._warn("Please enter your full name.");
      if (!this.regData.email)
        return this._warn("Please enter your email address.");
      if (!this.regData.email.includes("@"))
        return this._err("Please enter a valid email address.");
      if (
      ...
      if (!this.regData.password)
        return this._warn("Please create a password.");
      if (this.regData.password.length < 6)
        return this._err("Password must be at least 6 characters long.");
      if (this.regData.password !== this.regData.confirmPassword)
        return this._err("Passwords do not match.");
      ...
      try {
        await authAPI.register({
          ...
          service_type: this.regData.serviceType || null,
          business_name: this.regData.fullName,
        });
        ...
        try {
          ...
          if (login?.user) {
            ...
            await Swal.fire({
              icon: "success",
              title:
                SUCCESS_MESSAGES[login.user.role] || "Welcome to CampusSwap!",
              text: `Registered as ${login.user.full_name}.  Your account is status is: "pending admin verification" . You can browse the platform, but the "Verified" badge appears once an admin approves your account`,
              timer: 1800,
        ...
        console.warn(
          "[register] API register failed, using in-memory:",
          apiErr.message,
        );
      }
      ...
      const existingUser = this.registeredUsers.find(
        (u) => u.email === this.regData.email,
      );
      if (existingUser)
        return this._err(
          "This email is already registered in this browser session.",
        );
      ...
      await Swal.fire({
        icon: "success",
        title:
          SUCCESS_MESSAGES[roleMap[this.regData.role]] ||
    ...
    async handleLogin() {
      ...
      // Login is backend-only. No demo fallback, no mock users array.
      // If the API rejects us, we show the error and stop.
      try {
        ...
        console.error("[login] failed:", apiErr.message);

        await Swal.fire({
          icon: "error",
          title: "Login Failed",
          text:
            apiErr.message || "Check your email and password and try again.",
          confirmButtonColor: "#d33",
        });
      }
    },
...
.login-provider .login-right,
.login-admin .login-right,
.login-resmanager .login-right {
  border-top: 4px solid #2e7d5a;
}
.login-provider .btn-primary,
.login-admin .btn-primary,
.login-resmanager .btn-primary {
  background: #2e7d5a;
  color: #ffffff;
}
.login-provider .gold-text,
.login-admin .gold-text,
.login-resmanager .gold-text {
  color: #f5b941;
}
...
.verified-badge-preview.pending {
  background: #fff8e7;
  border-color: #f5b941;
}
