<template>
  <main class="marketplace-page">
    ...
    <p v-if="loadMessage && activeTab === 'marketplace'" class="listing-status">
      {{ loadMessage }}
    </p>
    <ProductGrid
...
/**
 * Match a product against a category pill key.
 * The DB stores listing_type as sale / rent / swap / sale_and_swap,
 * and categories as text ("Furniture", "Books & Textbooks", ...).
 * The pill keys are UI shorthand — this maps them.
 */
function matchesCategory(product, key) {
  if (key === "all") return true;
  if (key === "sell")
    return product.listingType === "sale" || product.listing_type === "sale";
  if (key === "rent")
    return product.listingType === "rent" || product.listing_type === "rent";
  if (key === "swap")
    return product.listingType === "swap" || product.listing_type === "swap";
  if (key === "furniture") return product.category === "Furniture";
  return true;
}
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((product) => {
    if (!matchesCategory(product, category.value)) return false;
    if (!q) return true;
    const searchable = [
      product.name,
      product.title,
...
async function loadListings() {
  try {
    ...
    if (!products.value.length)
      loadMessage.value = "No listings are available yet.";
  } catch {
    products.value = [];
    loadMessage.value =
      "Listings coming soon — we could not connect to the listings service.";
  }
}
...
.marketplace-actions::-webkit-scrollbar {
  display: none;
}
.action {
